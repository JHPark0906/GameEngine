...
public:
    ...
    [[nodiscard]] bool IsVisible(const Core::Aabb3D& localBounds, const Math::Matrix4x4& localToWorld) const;

    /// <summary>
    /// 절두체와 타일 local z=0 평면의 교차를 감싸는 local XY 영역이다.
    /// 교차가 없으면 빈 상자, 유한한 역변환을 구할 수 없으면 nullopt(전체 범위 사용)다.
    /// </summary>
    [[nodiscard]] std::optional<Core::Aabb2D> GetVisiblePlaneBounds(const Math::Matrix4x4& localToWorld) const;
